...
export default new Command({
  ...
  async execute(client, interaction) {
    ...
    if (queue && queue.tracks.length > 1) {
      const formattedQueue = queue.tracks.map((song, i) => {
        return `**${i}**. \`${song?.durationMS}\` - \`${song?.duration}\``;
      });
      ...
      const embeds = chunk(formattedQueue, 10).map((chunk) => {
        ...
        return new MessageEmbed({
          title: "Music",
          footer: {
            text: `${queue?.tracks.length} song(s) in queue | ${queue?.totalTime} total length`,
          },
          color: "#ffc600",
          fields: [
            {
