...
import { embed, random } from "../utils/helpers";
...
export default new Command({
  ...
  async execute(_, message, args) {
    if (args.length == 0) {
      ...
      return message.channel.send(embed({ title: "Rolled", description: number.toString() }));
    }
    const choice = random(args);
    return message.channel.send(embed({ title: "Rolled", description: choice }));
  },
});
